...
var map = {
    bitly: {
        'hosts': ['bit.ly', 'j.mp', 'ericri.es', 'jc.is', 'nyti.ms', 'rww.to', 'linkd.in', 'win.gs', 'dai.ly', 'imgry.net', 'mln.im', 'theatln.tc'],
        'active': true
    },
    isgd: {
        'hosts': ['is.gd'],
    ...
    snipurl: {
        'hosts': ['snipurl.com', 'snurl.com', 'snurl.com', 'cl.lk', 'snipr.com', 'sn.im'],
        'active': true
    }
};
...
var Unshorteners = {
    ...
    __request: function (url, cb) {
        request.get(url, { url:url, jar: false }, function(e,r,b) {
            if ( !e && r.statusCode && r.statusCode >= 200 && r.statusCode < 300 ) {
                ...
                if (!e) {
                    var e = new Error('Resolution failed');
    ...
    generic: function (url, callback) {
        ...
        request.head({ url:url, timeout:10000, headers:headers, jar: false }, handle);
    }
