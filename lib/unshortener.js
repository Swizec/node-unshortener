...
exports.config = function(opts) {
   if (typeof(opts.memcache) !== 'undefined') {
        if (!memcached) throw new Error("Unshortener requires optional dependency 'memcached' to use memcache");
        if (!hash) throw new Error("Unshortener requires optional dependency 'node_hash' to use memcache");
...
var Unshorteners = {
            ...
            if (error) {
                ...
                if (data.status_code && data.status_code >= 200 && data.status_code < 300){
                    ...
                    if (result.long_url) {
                        ...
                        if (options.recurseAfterAPICall) {
                            Unshorteners.generic(eurl, callback);
                        }
                        else {
                            callback(null, eurl);
                        }
    ...
    generic: function (url, callback) {
        var cookie_jar = request.jar();
        var headers = (url.host === "t.co") ? {} : {'User-Agent': (options.userAgent ? options.userAgent : 'AppleWebKit/525.13 (KHTML, like Gecko) Safari/525.13.')};
        var handle = function (err, res, secondAttempt) {
            if ( !err && res.statusCode && res.statusCode >= 200 && res.statusCode < 300 ) {
                ...
                if ( eurl.href.replace(/[\#\/]$/,'').toLowerCase() === url.href.replace(/[\#\/]$/,'').toLowerCase() ) {
                    // they differ only in trailing slash - return original
                    exports.saveUrl(url, url);
                    callback(null, url);
                } else if (  url.href.replace(/^(.+)(\?.+)$/, "$1/$2") === eurl.href ) {
                    // they differ only in trailing slash before query string - return original
                    exports.saveUrl(url, url);
                    callback(null, url);
                } else {
                    var pn = url.pathname;
                    ...
                    if (url.hostname === res.request.uri.hostname && pn === rpn) {
                        // they differ only in protocol, port, query string, and/or hash - return original
                        exports.saveUrl(url, url);
                        callback(null, url);
                    } else {
                        exports.saveUrl(url, eurl);
                ...
                if (res && res.request && res.request.uri) {
                    // we may have sucessfully followed redirects to a new URL, so retry with that URL or pass back to error handler
                    url = res.request.uri;
                }

                if (!err) {
                    if (!secondAttempt && res.statusCode === 405) {
                        return request.get({ url:url, timeout:10000, headers:headers, jar: cookie_jar }, handleAgain);
                    }
                    err = new Error('Resolution failed');
                    err.statusCode = res.statusCode;
                }
                else if (!secondAttempt && err.code === 'HPE_INVALID_CONTENT_LENGTH') {
                    // some servers respond to HEAD requests with the invalid header "Content-Length: -1"
                    return request.get({ url:url, timeout:10000, headers:headers, jar: cookie_jar }, handleAgain);
                }
        ...
        var handleAgain = function (err, res) {
            handle(err, res, true);
        };
